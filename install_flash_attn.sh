...
if [ "${FORCE_REINSTALL}" = "1" ]; then
    MAX_JOBS="${MAX_JOBS_VALUE}" uv pip install --reinstall --no-build-isolation "${FLASH_ATTN_SPEC}"
else
    MAX_JOBS="${MAX_JOBS_VALUE}" uv pip install --no-build-isolation "${FLASH_ATTN_SPEC}"
fi
